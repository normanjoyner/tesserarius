...
module.exports = {
    ...
    add_rule: function(chain, rule, fn){
        ...
        Joi.validate(rule, schema, function(err){
            ...
            if(_.has(rule, "destination")){
                command.push("-d");
                command.push(rule.destination);
            }

            if(_.has(rule, "source_mac")){
                command.push("-m mac");
                command.push("--mac-source");
                command.push(rule.source_mac);
            }

            if(_.has(rule, "state")){
                command.push("-m state");
                command.push("--state");
                command.push(rule.state.join(","));
            }
